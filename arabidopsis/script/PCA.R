...
pch1 <- rep((1:16),each=3)

pdf(pca_output, hei=7, wid=7)
layout(matrix(c(1,2),nrow=1), wid=c(5, 2))
par(mar=c(5,4,4,0))
par(xpd=TRUE)
plot(x[,1], x[,2], xlim=range(x[,1])*1.1, ylim=range(x[,2])*1.1, col=cols, pch=pch1, cex=1, type='p',
...
par(mar=c(5,0,4,0))
plot.new()
legend('left', unique(config$groups), pch = 1:16, col=set2_cols, bty='n')
dev.off()
